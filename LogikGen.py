...
### Changelog #########
#######################
## 2.021 * PREEXEC f�r hsb hinzugef�gt
##       * TagList.GrpListAll hinzugef�gt
...
class LogikGeneratorClass:
            ...
            try:
                if _lcmd.startswith("quit") or _lcmd.startswith("exit"):
                    ...
                    if os.path.isfile("{0}.hsb".format(_filename)):
                        _raw = raw_input(("Datei {0}.hsb existiert bereits. �berschreiben (j/n/a)?".decode("latin1").encode(sys.stdout.encoding)).format(_filename)).strip().lower()
                        if _raw == "a":
                            continue
                        if _raw == "n":
                    ...
                    if os.path.isfile("{0}.hsl".format(_filename)):
                        _raw = raw_input("Datei {0}.hsl existiert bereits. �berschreiben (j/n/a)?".format(_filename)).strip().lower()
                        if _raw == "a":
                            continue
                        if _raw == "n":
                            _filename += "_{0}".format(time.strftime("%Y-%m-%d_%H_%M"))
                    ...
                    console_info("'names [suchstring ... mit ! negiert]' zeigt die Namen der Ein-/Ausg�nge/Speicher an")
                    console_info("'files [-t (filetype) /-s (Gr��e)] [suchstring]' zeigt den Inhalt des ExtDatUrl (/opt) an")
                    ...
                    console_info("'-- print und import k�nnen auch ohne vorangestelltes exec ausgef�hrt werden")
                    ...
                    console_info("-- kann auch mit en1= abgek�rzt werden")
                    console("\n")
    ...
    def _HSLparser(self,lines,add_encrypted=False):
        ...
        if lines[0].find("coding:") == -1 and not add_encrypted:
            console_error("*** Warnung *** Encoding Zeile f�r iso-8859-1 fehlt")
                    ...
                    if _formel.startswith("eval(compile"):
                        ...
                        if _b64:
                            try:
                                ...
                                console_error("Base64code konnte nicht entschl�sselt werden")
    ...
    def load(self,fname):
        ...
        console_info("Bausteingr��e {0} KB".format(float(len("\n".join(_lines)))/1024))
...
if __name__ == "__main__":
    ...
    try:
        ...
        console_info("g�ltige Optionen sind")
        console_info("-d (--debug) startet den Debugger (immer benutzen)")
        console_info("-i xxxxx_logik.hsl (--import=) importiert eine .hsl Datei")
        console_info("-o logik.hsb (--open=) l�d eine .hsb Datei")
        console_info("-a [0|1] (--autorun=) startet die Ausf�hrung automatisch (ohne run)")
